...
fit_obj=[];
fit_obj_gof=[];
% phi = arrayfun(@(x)x.L_reg_timeave./x.L_amp,analysis_mat);
phi = arrayfun(@(x)x.L_reg./x.L_end,analysis_mat);
...
[binned.y,binned.x,binned.sem]=bin_average(bin_x(bin_x>-0.2&bin_x<0.6),bin_y(bin_x>-0.2&bin_x<0.6),15);

% plot binned average without fitting
hold on;
cl0 = lines(30);
errorbar(binned.x,binned.y,binned.sem,'-o','color',cl0(1,:),'linewidth',2,'CapSize',10)
...
% xdata = adjust_neg(binned.x-E0);
% ydata = binned.y;

xdata = adjust_neg(remove_nan(bin_x(bin_x<=0.6)));
ydata = remove_nan(bin_y(bin_x<=0.6));
