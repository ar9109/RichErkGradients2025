function [length_mat,seeds,fiducial,labeled_seeds,fiducial_fullimage,imQCBFHere,imQCosxHere] = segment_initial_seeds(length_mat,fish,s,BF,osx,PB_bw,dpa_ls,options)
arguments
    length_mat
    fish
    s
...
mask_out_closed = imclose(mask_out,strel('disk',50));
CC = bwconncomp(mask_out_closed);
numPixels = cellfun(@numel,CC.PixelIdxList);
[~,maxidx] = max(numPixels);
mask_out_closed = false(size(mask_out_closed));
mask_out_closed(CC.PixelIdxList{maxidx}) = 1;
mask_out(~mask_out_closed) = 0;
...
for raynum = 1:numel(CC.PixelIdxList)
    ...
    [arcBW,arcLHere,~,~,BW_oneray_small_closed,row_excerpt_QC,col_excerpt_QC] = measure_one_ray(BW_oneray,verbose = options.verbose,initial_seeds = 1);
    % figure; imshow(BW_oneray_small);
    %% plot QC overlaid image
    if ~isempty(options.verbose)&&options.verbose
        figure;
        imshowpair(BW_oneray_small_closed,arcBW)
    end
    [arc_row,arc_col] = find(arcBW);
    original_coor_row = arc_row+row_excerpt_QC(1)-1;
    original_coor_col = arc_col+col_excerpt_QC(1)-1;
    % paint
