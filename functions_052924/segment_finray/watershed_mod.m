function L = watershed_mod(varargin)
...
[A,seeds,conn] = parse_inputs(varargin{:});

cc = bwconncomp(seeds, conn);

conn = images.internal.getBinaryConnectivityMatrix(conn);
L = images.internal.builtins.watershed_meyer(A,conn,cc);


%--------------------------------------------------------------------
function [A,seeds,conn] = parse_inputs(varargin)

narginchk(2,3);

A = varargin{1};
validateattributes(A,{'uint8','uint16','single','double', 'logical'},...
              {'real', 'nonsparse'}, mfilename, 'A', 1);
seeds = varargin{2};
validateattributes(seeds,{'uint8','uint16','single','double', 'logical'},...
              {'real', 'nonsparse'}, mfilename, 'A', 1);
