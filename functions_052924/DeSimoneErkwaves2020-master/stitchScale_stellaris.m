function [myScale,stitched] = stitchScale_stellaris(paths,basename,opts)
...
if(isfield(opts,'inFolders'))
    if(opts.inFolders)
        myScale=parse_scale_folder_stellaris([inFolder folderName filesep],basename,opts);
    else
        myScale=parse_scale_folder_stellaris([inFolder filesep],basename,opts);        
    end
else
        myScale=parse_scale_folder_stellaris([inFolder filesep],basename,opts);        
end
...
if(~isfield(opts,'stitchMethod'))
    ...
    stitched=stitchStacksCoordCoor2_stellaris(myScale,opts,paths);%refCh,targetVox,cropImage,verbose);
else

    if(strcmp(opts.stitchMethod,'3DsinglePlane'))
        stitched=stitchStacksCoordCoor2_stellaris(myScale,opts,paths);%refCh,targetVox,cropImage,verbose);
    elseif(strcmp(opts.stitchMethod,'3Dproj'))
        stitched=stitchStacksCoordCoor2Proj(myScale,opts,paths);%refCh,targetVox,cropImage,verbose);
    end
end
