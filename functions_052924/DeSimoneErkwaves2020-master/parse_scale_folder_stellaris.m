function [mystack] = parse_scale_folder_stellaris(folder,basename,opts)
...
% 
%    % time
    posT=regexp(name,'_time'); % edited 111022
%     posT=regexp(name,'_t');
    if(~isempty(posT))
...
if(isnan(Position_list(1)))
    ...
    for p=1:numel(Position_list)
        metadata_dir=dir(strcat([folder 'MetaData/'],[basenameMetadata '_' basenameMetadata '_Position ' num2str(Position_list(p))],'.xml')); % need to be changed for new lasX format, edited 030624
