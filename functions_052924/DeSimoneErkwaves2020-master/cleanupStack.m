function [stackOut,roi,roiTight] = cleanupStack(stackIn,roi,opts)
...
if (isfield(opts,'fast')) && opts.fast % edited 120822 to allow using faster code
    [stack,roi,roiTight] = selectROIstack_fast(stack,roi,opts);
else
    [stack,roi,roiTight] = selectROIstack(stack,roi,opts);
end

% below for clean up each planes, not needed
% adj=[0 0.1];
% size_old=size(stack);
